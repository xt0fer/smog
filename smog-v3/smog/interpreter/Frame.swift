...
class Frame: SArray {
    ...
    func stackElement(idx: Int, put obj: SObject) {
        self.stack.indexableFields[idx] = obj
    }
    //
    //  local: index = (
    ...
    //  )
    func local(idx: Int) -> SObject {
        return self.stack.indexableFields[localOffset + idx - 1]
    }
    func local(idx: Int, put obj: SObject) {
        self.stack.indexableFields[localOffset + idx - 1] = obj
    }
    ...
    //  )
    func argument(idx: Int) -> SObject {
        return self.stack.indexableFields[idx]
    }
    func argument(idx: Int, put obj: SObject) {
        self.stack.indexableFields[idx] = obj
    }
    ...
    //  )
    func argument(idx: Int, at: Int) -> SObject {
        return self.context(level: at).argument(idx: idx)
    }
    func argument(idx: Int, at: Int, put obj: SObject) {
        var ctx = self.context(level: at)
        ctx.stack.indexableFields[idx] = obj
    }
    ...
    //  )
    func copyArgumentsFrom(frame: Frame) {
        //    "copy arguments from frame:
        //     - arguments are at the top of the stack of frame.
        //     - copy them into the argument area of the current frame"
        let numArgs = self.method.numberOfArguments()
        for i in 0...numArgs {
            self.stack.indexableFields[i+1] = frame.stackElement(idx: numArgs - 1 - i)
        }
    }
    //
    //  printStackTrace = (
